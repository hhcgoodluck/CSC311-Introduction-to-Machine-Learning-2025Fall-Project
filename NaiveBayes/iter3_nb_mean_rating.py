...
def process_multiselect(series, target_tasks):
    ...
    Missing responses are treated as selecting nothing.
    """
    processed = []
...
def main():
    ...
    academic_raw = df["How likely are you to use this model for academic tasks?"].apply(
        extract_rating
    )
    subopt_raw = df[
    ...
    # Multinomial Naive Bayes（baseline：alpha=1.0 Laplace Smooth）
    ...
    print(f"Training accuracy (NB, student-wise split, imputed): {train_acc:.3f}")
    print(f"Test accuracy (NB, student-wise split, imputed): {test_acc:.3f}")
